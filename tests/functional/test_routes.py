...
def test_create_recipe(testing_client):
    ...
    response = testing_client.post(
        '/', json={'name': 'recipe1', 'ingredients': 'ingredient1', 'steps': 'step1', 'rating': 1, 'favorite': False})
    assert response.status_code == 200
...
def test_create_recipe_wrong_rating(testing_client):
    ...
    """
    response = testing_client.post(
        '/', json={'name': 'recipe1', 'ingredients': 'ingredient1', 'steps': 'step1', 'rating': 10, 'favorite': False})
    assert response.status_code == 200
...
def test_update_recipe(testing_client):
    ...
    response = testing_client.put(
        '/1', json={'name': 'recipe', 'ingredients': 'ingredient', 'steps': 'stepsss', 'rating': 1, 'favorite': True})
    assert response.status_code == 200
