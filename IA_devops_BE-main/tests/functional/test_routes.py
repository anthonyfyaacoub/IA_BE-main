...
def test_create_recipe(testing_client):
    response = testing_client.post(
        '/', json={'name': 'recipe', 'ingredients': 'ingredient', 'steps': 'step', 'rating': 5, 'favorite': False})
    assert response.status_code == 200


def test_create_recipe_invalid_rating(testing_client):
    response = testing_client.post(
        '/', json={'name': 'recipe', 'ingredients': 'ingredient', 'steps': 'step', 'rating': 10, 'favorite': False})
    assert response.status_code == 200
...
def test_update_recipe(testing_client):
    response = testing_client.put(
        '/1', json={'name': 'recipe', 'ingredients': 'ingredient', 'steps': 'stepsssss', 'rating': 1, 'favorite': False})
    assert response.status_code == 200
